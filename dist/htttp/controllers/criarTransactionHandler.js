...
async function criarTransacaoHandler(req, res) {
    try {
        const { client, callbackUrl } = req.body;
        if (!client?.name || !client?.email || !client?.phone) {
            res.status(400).json({ error: 'Campos obrigatórios estão faltando' });
            return;
        }
        console.log('Iniciando criação de transação Pix...');
        const transacao = await (0, criarTransa__o_1.criarTransacaoPix)(client, process.env.PUBLIC_KEY || '', process.env.SECRET_KEY || '', callbackUrl);
